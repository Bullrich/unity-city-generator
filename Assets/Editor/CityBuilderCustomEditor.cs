    ...
    public class CityBuilderCustomEditor : Editor
    {
        ...
        */
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("by Javier Bullrich, Juan Cruz Araujo, Iván Ramos - © 2017",
        ...
        */
        ...
        void SetGameObjects()
        {
            ...
            _target.xStreets =
                (GameObject) EditorGUILayout.ObjectField("...Horizontal Streets", _target.xStreets, typeof(GameObject),
                    false);
            _target.zStreets =
                (GameObject) EditorGUILayout.ObjectField("...Vertical Streets", _target.zStreets, typeof(GameObject),
                    false);
            _target.crossRoad =
                (GameObject) EditorGUILayout.ObjectField("...Cross-roads", _target.crossRoad, typeof(GameObject),
                    false);
            if (_target.xStreets == null || _target.zStreets == null || _target.crossRoad == null)
                EditorGUILayout.HelpBox("You must set all three GameObjects.", MessageType.Error);
        }
        ...
        void SetCityParams()
        {
            ...
            else if (_target.mapWidth == 0)
                EditorGUILayout.HelpBox(
                    "Map Width must not be zero. A city of width 0 is literally nothing, don't get metaphysical with me!",
                    MessageType.Error);

            if (_target.mapHeight < 0)
                EditorGUILayout.HelpBox("Map Height must be a positive value.", MessageType.Error);
            else if (_target.mapWidth == 0)
                EditorGUILayout.HelpBox(
                    "Map Height must not be zero. A city of height 0 is literally nothing, don't get metaphysical with me!",
                    MessageType.Error);
        }
        ...
        void ConfigureNeighborhoods()
        {
            ...
            for (int i = 0; i < _target.neighborhoods.Length; i++)
            {
                ...
                _target.neighborhoods[i].ChanceToAppear = (int) EditorGUILayout.Slider("Chance To Appear",
                    _target.neighborhoods[i].ChanceToAppear, 0, 100);
                if (_target.neighborhoods[i].ChanceToAppear == 0)
                    EditorGUILayout.HelpBox("Keeping Chance to Appear at 0 will make this Neighborhood never appear.",
                        MessageType.Warning);
                for (int j = 0; j < _target.neighborhoods[i].buildings.Length; j++)
                {
                    _target.neighborhoods[i].buildings[j] = (GameObject) EditorGUILayout.ObjectField(
                        "Building #" + (i + 1),
                        _target.neighborhoods[i].buildings[j], typeof(GameObject), false);
                }
                if (_target.neighborhoods[i].buildings.Length == 0)
            ...
            {
                _target.neighborhoods[_target.neighborhoods.Length] = new Neighborhood();
            }
            /*
